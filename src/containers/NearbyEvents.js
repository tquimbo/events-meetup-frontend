...
const NearbyEvents = ({ loading, events, error }) => {
  ...
  const [limit, setLimit] = useState(20); // Number of events to fetch at once
  const [offset, setOffset] = useState(0); // Offset for API call
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (ip) {
        dispatch(fetchNearbyEvents(ip, limit, offset));
      } else {
